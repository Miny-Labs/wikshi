...
- Local build health is good: compile succeeds and tests pass (`325 passing, 0 failing`).
- Recent high-priority fixes (V-001/V-002, V-003, V-004) are present and covered by tests.
- Two high-risk correctness issues remain in `WikshiCreditOracle` around cross-chain event decoding (Aave/Compound topic handling and Gluwa `LoanExpired` selector mismatch risk).
...
- `npx hardhat test` -> success (`325 passing`)
...
**Status: FIXED** — `borrowerTopicIdx` is now per-signature configurable via `setSupportedEventSignature`, and tests confirm topic-index extraction for Aave V3 Repay (`topics[2]`) and PaymentMade (`topics[1]`).
...
**Status: FIXED** — Source allowlist is now `mapping(bytes32 chainKey => mapping(address => bool))` and `setApprovedSourceContract` takes `chainKey` parameter. Tests confirm per-chain scoping.
...
**Status: PARTIALLY FIXED** — Tests now cover borrowerTopicIdx for Aave V3 Repay (`topics[2]`) and PaymentMade (`topics[1]`), plus boundary checks for invalid topic indices.
...
- **Do not claim production-grade readiness yet** until F-02 is fixed and regression-tested.
- F-01 and F-04 have been addressed. F-05 is partially addressed.

## 10) Priority Remediation Plan

1. ~~Fix signature-specific borrower decoding (Aave/Compound) and add ABI-accurate tests.~~ **DONE**
...
5. ~~Add chain-scoped source trust model in next oracle version.~~ **DONE**
...
- Aave: https://aave.com/docs/aave-v3/smart-contracts/pool
- Morpho: https://docs.morpho.org/learn/concepts/market/
- Euler: https://docs.euler.finance/developers/evk/
- Maple: https://maplefinance.gitbook.io/maple/technical-resources/protocol-overview
- Goldfinch: https://dev.goldfinch.finance/docs/reference/how-the-protocol-works
- 3Jane: https://docs.3jane.xyz/introduction

## 12) Parallel Deep Research Addendum
