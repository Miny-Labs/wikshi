...
abstract contract USCBase {
    ...
    /// @dev Override in derived contracts to handle verified cross-chain events.
    /// @param action The action type specified by the caller.
    /// @param queryId Unique identifier for the processed query.
    /// @param encodedTransaction The RLP-encoded transaction from the source chain.
    /// @param chainKey The USC chain key identifying which source chain the proof came from.
    function _processAndEmitEvent(uint8 action, bytes32 queryId, bytes memory encodedTransaction, uint64 chainKey) internal virtual;
    ...
    function execute(
        ...
        // Forward chainKey so derived contracts can scope validation per-chain
        _processAndEmitEvent(action, queryId, encodedTransaction, chainKey);
